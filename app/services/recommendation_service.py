...
def calculate_user_similarity(user1: User, user2: User) -> float:
    ...
    # Interests similarity (highest weight)
    weight = 3.0
    total_weight += weight
    ...
    if user1.birth_date and user2.birth_date:
        ...
        # Age difference penalty decreases as users get older
        age_diff_ratio = 1 - min(abs(age1 - age2) / 20.0, 1.0)  # Max 20 years difference
    ...
    if user1.city and user2.city:
        if user1.city.lower() == user2.city.lower():
            score += weight
        elif user1.country and user2.country and user1.country.lower() == user2.country.lower():
            score += weight * 0.5
    ...
    # Gender similarity (lowest weight)
    weight = 0.5
    total_weight += weight
...
def get_user_activity_preferences(user_id: int) -> Dict[int, float]:
    ...
    for vote in user_votes:
        ...
        for position, activity_id in enumerate(ranking):
            # Score decreases as position increases (inverse ranking)
...
def find_similar_users(user: User, min_similarity: float = 0.3) -> List[Tuple[User, float]]:
    """
    ...
    # TODO: Replace with proper user repository query
    # For now, we'll use a small set of test users
    # Get test users and convert dict to User model
...
def get_collaborative_recommendations(
    ...
    # Get activity preferences for all similar users
    all_preferences = {}
    for similar_user, similarity in similar_users:
        ...
        for activity_id, score in user_prefs.items():
            ...
            if activity_id in all_preferences:
                all_preferences[activity_id] = max(all_preferences[activity_id], weighted_score)
            else:
                all_preferences[activity_id] = weighted_score
    
    # Score current activities based on collaborative filtering
    scored_activities = []
    for activity in current_activities:
        base_score = all_preferences.get(activity.id, 0.0)
        
        # Boost score based on activity type matching user interests
        ...
        # Apply indoor/outdoor preference boost
