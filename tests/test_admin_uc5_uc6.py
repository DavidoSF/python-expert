...
def test_admin_add_custom_activity():
    ...
    activity_data = {
        ...
        "type": "community",  # Valid: cultural, sports, community, other
        "location": "Paris",
        "is_indoor": False,
        "date": "2025-11-10",
...
def test_admin_list_activities_requires_admin():
    ...
    # Try with non-existent user (should fail)
    response = client.get(
        "/admin/activities",
        params={
            "city": "Paris",
...
def test_admin_list_activities_with_admin_user():
    ...
    # Use user_id 3 (Carol, who is an administrator in users.json)
    response = client.get(
        "/admin/activities",
        params={
            "city": "Paris",
...
def test_admin_view_configuration():
    ...
    # Use admin user_id 3 (Carol)
    response = client.get(
        "/admin/config",
        params={"admin_user_id": 3}  # Carol is administrator
    )
    assert response.status_code == 200
    ...
    print(f"  - Confidence threshold: {config['recommendations']['confidence_threshold']}")
...
def test_config_view_requires_admin():
    ...
    response = client.get(
        "/admin/config",
        params={"admin_user_id": 1}  # Alice is subscriber, not admin
    )
    assert response.status_code == 403
